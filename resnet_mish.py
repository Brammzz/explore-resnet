...
class MishResidualBlock(nn.Module):
    """ResidualBlock with Mish activation instead of ReLU"""
    ...
    def forward(self, x):
        identity = x
        out = self.mish(self.bn1(self.conv1(x)))  # Replace ReLU with Mish
        out = self.bn2(self.conv2(out))
        if self.proj is not None:
            identity = self.proj(identity)
        out = self.mish(out + identity)  # Replace ReLU with Mish
        return out

class MishResNet34(nn.Module):
    """ResNet-34 with Mish activation function"""
    def __init__(self, num_classes):
        ...
        self.stem = nn.Sequential(
            ...
            nn.BatchNorm2d(64),
            self.mish,  # Replace ReLU with Mish
    ...
    def _make_layer(self, in_c, out_c, n_blocks, stride):
        blocks = [MishResidualBlock(in_c, out_c, stride=stride)]
        for _ in range(n_blocks-1):
            blocks.append(MishResidualBlock(out_c, out_c, stride=1))
...
def create_mish_resnet(num_classes=5):
    return MishResNet34(num_classes)
...
def plot_curves(history, save_dir):
    ...
    plt.title("Mish-ResNet-34 Accuracy Curve")
    ...
    plt.title("Mish-ResNet-34 Loss Curve")
...
def plot_confusion_matrix(y_true, y_pred, classes, save_dir):
    ...
    plt.title("Mish-ResNet-34 Confusion Matrix")
...
if __name__ == "__main__":
    ...
    SAVE_DIR = BASE_DIR / "mish_resnet_result"
    ...
    # Model with Mish activation
    model = create_mish_resnet(num_classes=num_class).to(device)
    ...
    print(f"✅ Training Mish-ResNet-34 selesai. Best Val Acc: {best_acc:.4f}")
