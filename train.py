...
class Trainer:
    """
    Complete training pipeline for Plain-34 and ResNet-34 models.
    Handles training, validation, metrics tracking, and visualization.
    """
...
def prepare_data(data_dir, batch_size=32, img_size=224):
    ...
    Args:
        data_dir: Path to dataset directory (should have 'train' and 'val' subdirs)
...
def main():
    ...
    # Train Plain-34
    print("\n" + "="*70)
    print("STAGE 1: TRAINING PLAIN-34")
    print("="*70)
    plain_model = create_plain34(num_classes=len(classes))
    plain_trainer = Trainer(plain_model, train_loader, val_loader, device, "Plain-34")
    plain_history = plain_trainer.train(NUM_EPOCHS, LEARNING_RATE)
    plain_trainer.plot_history('plain34_history.png')
    plain_trainer.save_history('plain34_history.json')
    
    # Train ResNet-34
    print("\n" + "="*70)
    print("STAGE 2: TRAINING RESNET-34")
    print("="*70)
    ...
    print("="*70)
    print(f"Plain-34:")
    print(f"  Final Train Acc: {plain_history['train_acc'][-1]:.2f}%")
    print(f"  Final Val Acc:   {plain_history['val_acc'][-1]:.2f}%")
    print(f"  Best Val Acc:    {max(plain_history['val_acc']):.2f}%")
    print(f"\nResNet-34:")
    ...
    print(f"\nImprovement:")
    improvement = max(resnet_history['val_acc']) - max(plain_history['val_acc'])
    print(f"  Val Acc Gain:    {improvement:+.2f}%")
    print("="*70)
