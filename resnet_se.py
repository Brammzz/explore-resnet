...
class SEResidualBlock(nn.Module):
    """ResidualBlock with Squeeze-and-Excitation"""
...
class SEResNet34(nn.Module):
    """ResNet-34 with Squeeze-and-Excitation Blocks"""
    def __init__(self, num_classes, reduction=16):
        ...
        self.layer2 = self._make_layer(64,  128, 4, stride=2, reduction=reduction)
        self.layer3 = self._make_layer(128, 256, 6, stride=2, reduction=reduction)
        self.layer4 = self._make_layer(256, 512, 3, stride=2, reduction=reduction)
        self.pool   = nn.AdaptiveAvgPool2d((1,1))
        self.fc     = nn.Linear(512, num_classes)
        self._init_weights()

    def _make_layer(self, in_c, out_c, n_blocks, stride, reduction):
        blocks = [SEResidualBlock(in_c, out_c, stride=stride, reduction=reduction)]
        for _ in range(n_blocks-1):
            blocks.append(SEResidualBlock(out_c, out_c, stride=1, reduction=reduction))
...
def create_se_resnet(num_classes=5, reduction=16):
    return SEResNet34(num_classes, reduction)
...
def plot_curves(history, save_dir):
    ...
    plt.title("SE-ResNet-34 Accuracy Curve")
    ...
    plt.title("SE-ResNet-34 Loss Curve")
...
def plot_confusion_matrix(y_true, y_pred, classes, save_dir):
    ...
    plt.title("SE-ResNet-34 Confusion Matrix")
...
if __name__ == "__main__":
    ...
    SAVE_DIR = BASE_DIR / "se_resnet_result"
    ...
    print(f"🏷️  Classes: {df['label'].value_counts()}")
    ...
    # Model with SE Block (reduction=16)
    model = create_se_resnet(num_classes=num_class, reduction=16).to(device)
    ...
    print(f"✅ Training SE-ResNet-34 selesai. Best Val Acc: {best_acc:.4f}")
